...
/**Function*************************************************************
  ...
  Synopsis    [Profiles rewrite candidates without modifying the AIG.]
...
***********************************************************************/
int Abc_NtkRewriteProfile( Abc_Ntk_t * pNtk, int fUpdateLevel, int fUseZeros, int fVerbose, int fVeryVerbose, int fPlaceEnable, int nKeepMax )
{
    ProgressBar * pProgress;
    Cut_Man_t * pManCut;
    Rwr_Man_t * pManRwr;
...
Rwr_ManAddTimeCuts( pManRwr, Abc_Clock() - clk );
    ...
    pProf = Rwr_ProfileStart( pNtk, nKeepMax, fVerbose );
    ...
    Abc_NtkForEachNode( pNtk, pNode, i )
    {
        ...
        if ( Abc_ObjFanoutNum(pNode) > 1000 )
            continue;
        Rwr_NodeRewriteProfile( pManRwr, pManCut, pNode, fUpdateLevel, fUseZeros, fPlaceEnable, pProf );
    }
    Extra_ProgressBarStop( pProgress );
Rwr_ManAddTimeTotal( pManRwr, Abc_Clock() - clkStart );

    Rwr_ProfileFinalize( pProf );
    Rwr_ProfilePrint( pProf );
    ...
    if ( !Abc_NtkCheck( pNtk ) )
    {
        printf( "Abc_NtkRewriteProfile: The network check has failed.\n" );
        RetValue = 0;
    }
    return RetValue;
}
